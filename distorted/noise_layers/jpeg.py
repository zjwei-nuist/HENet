...
class DDJS(JpegBasic):
    def __init__(self, threshold, subsample=0):
        super(DDJS, self).__init__()
        ...
        self.matrix = torch.zeros(1, 3, 8, 8)

        for Q in range(10, 91, 10):
            self.mat_path = '/home/zjw/HENet/distorted/noise_layers/mask/sum_matrix_B=' + str(Q) + '.mat'
            load_mat = scio.loadmat(self.mat_path)
            y = load_mat['y_matrix']
            cb = load_mat['cb_matrix']
            cr = load_mat['cr_matrix']
            y = (100 * (1 - y)) / 9.
            cb = (100 * (1 - cb)) / 9.
            cr = (100 * (1 - cr)) / 9.
            self.matrix[:, 0:1, :, :] += torch.tensor(y)
            self.matrix[:, 1:2, :, :] += torch.tensor(cb)
            self.matrix[:, 2:3, :, :] += torch.tensor(cr)

        show_matirx = np.asarray(self.matrix).astype('uint8')
        print('==============Mask without Threshold============')
        print(show_matirx[0][0])
        print(show_matirx[0][1])
        print(show_matirx[0][2])

        self.matrix_mask = self.matrix > threshold
        self.mask = torch.zeros(1, 3, 8, 8)
        self.mask[self.matrix_mask] = 1
        show_matirx = np.asarray(self.mask).astype('uint8')
        print('==============Mask with Threshold============')
        print(show_matirx[0][0])
        print(show_matirx[0][1])
        print(show_matirx[0][2])
    ...
    # def adaptive_round_mask(self, x, threshold):
    ...
    def adaptive_round_mask(self, x):
        mask = self.mask.repeat(1, 1, x.shape[2] // 8, x.shape[3] // 8).to(x.device)
        return x * mask
    ...
    def forward(self, image_and_cover):
        ...
        image_mask = self.adaptive_round_mask(image_dct)
